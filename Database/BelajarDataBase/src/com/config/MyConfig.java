...
public abstract class MyConfig {
    ...
    private static final String DB_URL = "jdbc:mysql://localhost:3306/ayyshop";
    ...
    public static void getDatabase() {
        ...
        try {
            ...
            resultset = statement.executeQuery("SELECT * FROM tb_baju ORDER BY ID DESC");
    ...
    public static void insertData() {
        ...
        try {
            PreparedStatement preparedStatement = connect
                    .prepareStatement("INSERT INTO `tb_baju` (`ID`, `Nama`, `Harga`, `Jumlah`) VALUES (NULL, ?, ?, ?)");
    ...
    private static void editNama(int id, String namaBaru) {
        ...
        try {
            PreparedStatement preparedStatement = connect.prepareStatement(
                    "UPDATE `tb_baju` SET `Nama` = ? WHERE `ID` = ?");
            preparedStatement.setString(1, namaBaru);
    ...
    private static void editHarga(int id, long hargaBaru) {
        ...
        try {
            PreparedStatement preparedStatement = connect.prepareStatement(
                    "UPDATE `tb_baju` SET `Harga` = ? WHERE `ID` = ?");
    ...
    private static void editJumlah(int id, int jumlahBaru) {
        ...
        try {
            PreparedStatement preparedStatement = connect.prepareStatement(
                    "UPDATE `tb_baju` SET `Jumlah` = ? WHERE `ID` = ?");
    ...
    public static void deleteData() {
        ...
        try {
            statement = connect.createStatement();
            statement.executeUpdate("DELETE FROM `tb_baju` WHERE `ID` = " + id);
